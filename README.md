...
### 1. 🏛️ 景点数据读取服务器 (`publisher/places_read_mcp.py`)
...
### 2. 🎨 图片生成服务器 (`publisher/generate_mcp.py`)
...
### 4. 🗺️ 路径规划服务器 (`publisher/route_planning_mcp.py`)
...
### 5. 🌤️ 天气查询服务器 (`publisher/weather_mcp.py`)
...
*   **Nano Banana API Key：** 访问 [acedata.cloud](https://acedata.cloud/) 注册并获取（需在 `publisher/generate_mcp.py` 中修改 `token` 变量）。
...
```json
{
  "mcpServers": {
    "amap-route-planning": {
      ...
      "args": ["你的项目路径\\publisher\\route_planning_mcp.py"],
      "cwd": "你的项目路径",
      "env": {
    ...
    "amap-weather": {
      ...
      "type": "stdio",
      "command": "你的Python解释器路径",
      "args": ["你的项目路径\\publisher\\weather_mcp.py"],
      "cwd": "你的项目路径",
      "env": {
        "AMAP_API_KEY": "你的高德地图API_Key"
      }
    ...
    "places_read": {
      ...
      "type": "stdio",
      "command": "你的Python解释器路径",
      "args": ["你的项目路径\\publisher\\places_read_mcp.py"],
      "cwd": "你的项目路径"
    },
    "image-generator": {
      ...
      "type": "stdio",
      "command": "你的Python解释器路径",
      "args": ["你的项目路径\\publisher\\generate_mcp.py"],
      "cwd": "你的项目路径"
    },
